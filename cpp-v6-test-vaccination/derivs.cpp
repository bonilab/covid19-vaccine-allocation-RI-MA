...
void derivs( double t, double *y, double *dydt)
{
    ...
    //
    double foi=0.0, foi_ac=0.0, rel_sus_z=0.0;
    ...
    for(i=0;i<DIMENSION;i++) dydt[i]=0.0; // zero everything out
    ...
    // natural immunity
    double trr = ((double)NUMRR + 1.0) / ppc->v[ i_len_natural_immunity ];
    double trrhosp = ((double)NUMRRHOSP + 1.0) / ppc->v[ i_len_natural_immunity ];

    // vaccination
    // transition rate from one Z class to another and from the last Z back to S
    double trz_1 = ((double)NUMZ_1) / ppc->v[ i_phi_vac1_protect_duration ];
    double trz_2 = 0;((double)NUMZ_2) / ppc->v[ i_phi_vac2_protect_duration ];
    ...
    //
    // waning of immunity -- moving R to S
    for (ac=0; ac<NUMAC; ac++){
        dydt[ac] += trr * y[STARTRR + (NUMRR-1)*NUMAC + ac];
        dydt[ac] += trrhosp * y[STARTRRHOSP + (NUMRRHOSP-1)*NUMAC + ac];
    }

    //
    // end of vaccine's duration of protection, i.e. moving from Z to S
    for(ac=0; ac<NUMAC; ac++){
        dydt[ac] += trz_1 * y[STARTZ_1 + (NUMZ_1-1)*NUMAC + ac];
        dydt[ac] += trz_2 * y[STARTZ_2 + (NUMZ_2-1)*NUMAC + ac];
    }
    ...
    if( !G_B_USESOCIALCONTACTMATRIX )
    {
        ...
        // vaccination
        if( foi > 0){
            double foi_div_pop = foi/ popsize;
            // double time_z;
            
            //// NOTE: assuming stage 1 of Z has perfect protection (i.e. ppc->v_efficacy_Z_x[0] = 1.0, relative_susceptibility or rel_sus_z = 0.0)
            ////       hence, we only move individuals from stage 2 (among NUMZ) and later to E
            
            // Z_1 to E
            for(i=1; i<NUMZ_1; i++){
                // rel_sus_z = Get_Rel_Sus_Z(foi/ popsize, ((double)i)/trz_1, ppc->v[ i_vac1_eff_halflife], ppc->v[ i_vac1_eff_slope]);
                // rel_sus_z = Get_Rel_Sus_Z_pow(foi/ popsize, ((double)i)/trz_1, ppc->v[ i_vac1_eff_halflife_pow_slope], ppc->v[ i_vac1_eff_slope]);

                // time_z = ((double)i)/trz_1;

                // fprintf(stderr, "rel_sus_z %f \n", rel_sus_z);
                for(ac=0; ac<NUMAC; ac++) {

                    // rel_sus_z = -log( ( ppc->v_rel_eff_Z_1[ac] * ppc->v_efficacy_Z_1[i] ) * (1.0 - exp(-foi_div_pop * time_z )) + exp(-foi_div_pop * time_z ) ) / (foi_div_pop * (time_z + 1.0e-30));
                    rel_sus_z = ppc->v_rel_sus_Z_1[ac][i];

                    dydt[STARTE + ac] += foi_div_pop * rel_sus_z * y[STARTZ_1 + i*NUMAC + ac] ;   
                    dydt[STARTZ_1 + i*NUMAC + ac] -= foi_div_pop * rel_sus_z * y[STARTZ_1 + i*NUMAC + ac] ;
                }
            }
            // Z_2 to E
            /* for(i=1; i<NUMZ_2; i++){
                // rel_sus_z = Get_Rel_Sus_Z(foi/ popsize, ((double)i)/trz_2, ppc->v[ i_vac2_eff_halflife], ppc->v[ i_vac2_eff_slope]);
                // rel_sus_z = Get_Rel_Sus_Z_pow(foi/ popsize, ((double)i)/trz_2, ppc->v[ i_vac2_eff_halflife_pow_slope], ppc->v[ i_vac2_eff_slope]);

                // time_z = ((double)i)/trz_2;

                for(ac=0; ac<NUMAC; ac++) {
                    
                    // rel_sus_z = -log( ( ppc->v_rel_eff_Z_2[ac] * ppc->v_efficacy_Z_2[i] ) * (1.0 - exp(-foi_div_pop * time_z )) + exp(-foi_div_pop * time_z ) ) / (foi_div_pop * (time_z + 1.0e-30));
                    rel_sus_z = ppc->v_rel_sus_Z_2[ac][i];

                    dydt[STARTE + ac] += foi_div_pop * rel_sus_z * y[STARTZ_2 + i*NUMAC + ac] ; 
                    dydt[STARTZ_2 + i*NUMAC + ac] -= foi_div_pop * rel_sus_z * y[STARTZ_2 + i*NUMAC + ac] ;
                }
            } */
        }
    ...
    if( G_B_USESOCIALCONTACTMATRIX )
    {
        ...
        // vaccination
        for(ac=0; ac<NUMAC; ac++) {
            foi_ac = ppc->v_rel_susc[ac] * ( foi_ofcomm[ac] + foi_ofhosp[ac] );
            if( foi_ac > 0){
                double foi_div_pop = foi_ac / popsize;
                // double time_z;
                ...
                // Z_1 to E
                for(i=1; i<NUMZ_1; i++){
                    // rel_sus_z = Get_Rel_Sus_Z(foi_ac / popsize, ((double)i)/trz_1, ppc->v[ i_vac1_eff_halflife], ppc->v[ i_vac1_eff_slope]);
                    // rel_sus_z = Get_Rel_Sus_Z_pow(foi_ac / popsize, ((double)i)/trz_1, ppc->v[ i_vac1_eff_halflife_pow_slope], ppc->v[ i_vac1_eff_slope]);
                ...
                // Z_2 to E
                /* for(i=1; i<NUMZ_2; i++){
                    // rel_sus_z = Get_Rel_Sus_Z( foi_ac / popsize, ((double)i)/trz_2, ppc->v[ i_vac2_eff_halflife], ppc->v[ i_vac2_eff_slope]);
                    // rel_sus_z = Get_Rel_Sus_Z_pow( foi_ac / popsize, ((double)i)/trz_2, ppc->v[ i_vac2_eff_halflife_pow_slope], ppc->v[ i_vac2_eff_slope]);
    ...
    for(ac=0;ac<NUMAC;ac++)
    {
        ...
        // transit out to the second R-class
        dydt[STARTR + ac] -= trr * y[STARTR + ac];
    }
    // 
    // transition through other R-classes
    for (ac=0; ac<NUMAC; ac++){
        for (i=0; i<NUMRR; i++){
            if(i==0) // 
            {
                dydt[STARTRR + i*NUMAC + ac] = trr * y[STARTR + ac] - trr * y[STARTRR + i*NUMAC + ac];
            }
            else // 
            {
                dydt[STARTRR + i*NUMAC + ac] = trr * y[STARTRR + (i-1)*NUMAC + ac] - trr * y[STARTRR + i*NUMAC + ac];
            }
        }
    ...
    for(ac=0;ac<NUMAC;ac++)
    {
        ...
        // transit out to the second RHOSP-class
        dydt[STARTRHOSP + ac] -= trrhosp * y[STARTRHOSP + ac];
        
    }
    // 
    // transition through other RHOSP-classes
    for (ac=0; ac<NUMAC; ac++){
        for (i=0; i<NUMRRHOSP; i++){
            if(i==0) // 
            {
                dydt[STARTRRHOSP + i*NUMAC + ac] = trrhosp * y[STARTRHOSP + ac] - trrhosp * y[STARTRRHOSP + i*NUMAC + ac];
            }
            else // 
            {
                dydt[STARTRRHOSP + i*NUMAC + ac] = trrhosp * y[STARTRRHOSP + (i-1)*NUMAC + ac] - trrhosp * y[STARTRRHOSP + i*NUMAC + ac];
            }
        }
    ...
    // ### 15 ### CUMULATIVE SYMPTOMATIC and TESTED INCIDENCE CLASSES
    for(ac=0;ac<NUMAC;ac++)
    {
        dydt[STARTJTEST + ac] = 0.0 ;
    }
    ...
    {
        // 4 Z_1 classes
        // for(i=0; i<NUMZ_1; i++)       dydt[STARTZ_1 + i*NUMAC + ac] = 0.0 ; 
        
        dydt[STARTZ_1 + ac] += -trz_1 * y[STARTZ_1 + ac] ;
        for(i=1; i<NUMZ_1; i++)
        {
            dydt[STARTZ_1 + i*NUMAC + ac] +=  trz_1 * y[STARTZ_1 + (i-1)*NUMAC + ac] - trz_1 * y[STARTZ_1 + i*NUMAC + ac];
        }

        // 4 Z_2 classes
        // for(i=0; i<NUMZ_2; i++)      dydt[STARTZ_2 + i*NUMAC + ac] = 0.0 ; 

        dydt[STARTZ_2 + ac] += -trz_2 * y[STARTZ_2 + ac] ;
        for(i=1; i<NUMZ_2; i++)
        {
            dydt[STARTZ_2 + i*NUMAC + ac] +=  trz_2 * y[STARTZ_2 + (i-1)*NUMAC + ac] - trz_2 * y[STARTZ_2 + i*NUMAC + ac];
        }
    }

    // ### 18 ### CUMULATIVE VACCINATED
    // 2020-11-30: dealing with vaccine rollout campaign only --> handle this class in generate_trajectories.cpp
    for(ac=0;ac<NUMAC;ac++)
    {
        dydt[STARTJZ_1+ac] = 0.0;
        dydt[STARTJZ_2+ac] = 0.0;
    }
