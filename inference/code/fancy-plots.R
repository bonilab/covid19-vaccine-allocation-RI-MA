...
traj.sim <- function(samples, odepath, csv = FALSE, odesim.version = "v5"){
  ...
  for (k in 1:n.samples){
    ...
    tp.k <- traj.fine.process(traj.k, loc = samples$loc, odesim.version = odesim.version)
...
traj.fine.process <- function(traj, loc, odesim.version="v5"){
  # Input:
  ...
  if(odesim.version=="v6"){
    discharge.idx=c(281:289,596:622) 
  }
...
summary.plots <- function(S, directory, odepath, plot.name, alpha, burnin=0,
                          sf.choice.save = FALSE, odesim.version="v5"){
